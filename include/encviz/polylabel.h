...
/**
 * \file
 * \brief polylabel — find the pole of inaccessibility (most distant internal point)
 * of a polygon, useful for optimal label placement.
 *
 * This is the C++ port of polylabel (https://github.com/mapnik/polylabel),
 * but ported to take the OGR geometry primatives as input
 *
 * ISC License
...
template <class T>
FlatPolygon<T> flatten(const OGRPolygon& polygon) {
    FlatPolygon<T> fp;
    ...
    fp.ringEnds.reserve(1 + polygon.getNumInteriorRings());

    for (const auto& ring : polygon)
    {
        for (const auto& p : ring) {
            fp.coords.push_back(p.getX());
            fp.coords.push_back(p.getY());
        }
...
template <class T>
OGRPoint polylabel(const OGRPolygon& polygon, T precision = 1, bool debug = false, T* distance = nullptr) {
    using namespace detail;

    // find the bounding box of the outer ring
    OGREnvelope envelope;
    polygon.getEnvelope(&envelope);
    const T minX = envelope.MinX;
    const T minY = envelope.MinY;
    const T maxX = envelope.MaxX;
    const T maxY = envelope.MaxY;
    ...
    FlatPolygon<T> fp = flatten<double>(polygon);
    ...
    return OGRPoint(bestCell.x, bestCell.y);
}
